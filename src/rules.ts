/**
 * The rule catalog. Codes are stable API: they are never renumbered or
 * repurposed, only added. Severity policy: E1xx means the config will not
 * load, will not do what it says, or a server silently cannot start; W2xx
 * means something is ignored, fragile or unsafe; I3xx is advisory.
 *
 * Per-finding messages are composed in the analyzer with the concrete
 * key/value; the catalog carries the rule's general story for
 * `plumbline explain` and docs/rules.md.
 */
...
export interface RuleInfo {
  code: string;
  severity: Severity;
  /** One-line summary, shown in docs and `explain`. */
  title: string;
  /** The longer story: why the client behaves this way, what to do. */
  detail: string;
}

const rule = (code: string, severity: Severity, title: string, detail: string): RuleInfo => ({
  code,
  severity,
  title,
  detail,
...
export const RULES: ReadonlyArray<RuleInfo> = [
  ...
  rule(
    "E102",
    "error",
    "comment in a strict-JSON config",
  ...
  rule(
    "E103",
    "error",
    "trailing comma in a strict-JSON config",
  ...
  rule(
    "E110",
    "error",
    "wrong top-level key for this client",
  ...
  rule(
    "E111",
    "error",
    "top-level key looks like a typo of the server container",
  ...
  rule(
    "E113",
    "error",
    "`inputs` is only understood by VS Code",
  ...
  rule(
    "E121",
    "error",
    "server has both `command` and `url`",
  ...
  rule(
    "E125",
    "error",
    "unknown server key that is a near-miss of a real one",
  ...
  rule(
    "E126",
    "error",
    "Claude Desktop cannot launch remote (url) servers from this file",
  ...
  rule(
    "E129",
    "error",
    "`envFile` is only understood by VS Code",
  ...
  rule(
    "E131",
    "error",
    "${input:...} has no matching entry in `inputs`",
  ...
  rule(
    "E132",
    "error",
    "${...} variables are not substituted by this client",
  ...
  rule(
    "W202",
    "warning",
    "duplicate key inside an object",
  ...
  rule(
    "W207",
    "warning",
    "relative path in command or args",
  ...
  rule(
    "W209",
    "warning",
    "Windows: npm-family shims need cmd /c",
  ...
  rule(
    "W210",
    "warning",
    "plaintext secret in the config",
  ...
  rule(
    "W211",
    "warning",
    "interpreter does not match the script extension",
  ...
  rule(
    "W212",
    "warning",
    "server name may confuse tool namespacing",
  ...
  rule(
    "W213",
    "warning",
    "remote URL over plain http to a non-loopback host",
  ...
  rule(
    "I301",
    "info",
    "JSONC niceties are fine here but not portable",
...
const BY_CODE = new Map(RULES.map((info) => [info.code, info]));

/** Look up a rule by code (case-insensitive). */
export function ruleByCode(code: string): RuleInfo | null {
  return BY_CODE.get(code.toUpperCase()) ?? null;
}

/** All rule codes, in catalog order. */
export function ruleCodes(): string[] {
  return RULES.map((info) => info.code);
}
